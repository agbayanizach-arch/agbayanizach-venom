...
async def on_member_join(member):
    ...
    # --- CUSTOM WELCOME GREET SYSTEM (from command(1).py) ---
    target_channel_id = welcome_settings["channel_id"]
    if target_channel_id:
        greet_channel = bot.get_channel(target_channel_id)
        if greet_channel:
            raw_msg = welcome_settings["message"]
            formatted_msg = raw_msg.replace("{member}", member.mention)
    ...
    try:
        ...
        if welcome_channel:
            await welcome_channel.send(
                f"📥 **{member.mention}** joined, but I couldn't check the server invites."
            )
        return

    # Find which invite was used by comparing its current use count
    # with the cached count from before the member joined.
    used_invite = None
    ...
    if member.id in history_db:
        if used_invite and used_invite.inviter:
            ...
            if welcome_channel:
                real = max(
                    0,
                    inviter_stats["regular"]
                    + inviter_stats["bonus"]
                    - inviter_stats["leaves"]
                    - inviter_stats["fake"]
                )
                await welcome_channel.send(
                    f"👋 **{member.mention}** rejoined the server using invite code "
                    f"`{used_invite.code}` (Invited by **{inviter.name}**, who now has "
                    f"`{real}` real invites)."
                )
        elif welcome_channel:
            await welcome_channel.send(
                f"👋 **{member.mention}** rejoined the server, but I couldn't trace the invite link."
            )
        return
    ...
    if used_invite and used_invite.inviter:
        ...
        if inviter.id == member.id:
            ...
            if welcome_channel:
                await welcome_channel.send(
                    f"📥 **{member.mention}** joined using their own invite code!"
                )
            return
        ...
        if account_age.days < 1:
            ...
            status_text = "⚠️ (New Account / Fake Risk)"
        else:
        ...
        if welcome_channel:
            await welcome_channel.send(
                f"📥 **{member.mention}** joined! Invited by **{inviter.mention}** "
                f"using code `{used_invite.code}`. They now have **{real_invites}** "
                f"real invites! {status_text}"
            )
    else:
        history_db.add(member.id)
        if welcome_channel:
            await welcome_channel.send(
                f"📥 **{member.mention}** joined the server, but I couldn't track an invite link."
            )
...
@app_commands.describe(message="Welcome message. Use {member} to mention the new member.")
...
async def testgreet(interaction: discord.Interaction):
    ...
    if channel is None:
        ...
        return
    msg = welcome_settings["message"].replace("{member}", interaction.user.mention)
