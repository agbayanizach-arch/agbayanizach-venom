...
# Animated custom Discord emoji used for giveaway entries.
GIVEAWAY_EMOJI = discord.PartialEmoji(
    name="tada",
    id=1046529384756291645,
    animated=True,
)
GIVEAWAY_EMOJI_TEXT = str(GIVEAWAY_EMOJI)  # <a:tada:1046529384756291645>

def is_giveaway_reaction(reaction):
    # Support both the new animated emoji and old giveaways that used 🎉.
    emoji = reaction.emoji
    return (
        emoji == GIVEAWAY_EMOJI
        or str(emoji) == GIVEAWAY_EMOJI_TEXT
        or str(emoji) == "🎉"
    )
...
async def finish_giveaway(giveaway_id: str):
    ...
    for reaction in giveaway_msg.reactions:
        if not is_giveaway_reaction(reaction):
            continue
        try:
...
async def reroll_giveaway(ctx, giveaway_id: str):
    # Only members with the Administrator permission in THIS server can use it.
    if ctx.guild is None or not isinstance(ctx.author, discord.Member) or not ctx.author.guild_permissions.administrator:
        await ctx.send(
            "❌ You need the **Administrator** permission in this server to use `-greroll`.",
            delete_after=5
        )
        return
