...
class VouchTargetView(discord.ui.View):
    def __init__(self, owner_id: int):
        ...
        # Discord's native User Select displays each member's profile/avatar.
        # The callback below still restricts the final choice to administrators.
        select = discord.ui.UserSelect(
            placeholder="Select the administrator you want to vouch for",
            min_values=1,
            max_values=1,
            custom_id=f"vouch_admin_user_select:{owner_id}"
        )
    ...
    async def target_select(self, interaction: discord.Interaction):
        ...
        if not isinstance(target, discord.Member):
            await interaction.response.send_message("❌ Please select a member from this server.", ephemeral=True)
            return

        # Only administrators can be vouched for.
...
class VouchResultView(discord.ui.View):
    ...
    async def submit_vouch(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            ...
            view=VouchTargetView(interaction.user.id),
            ephemeral=True
        )
