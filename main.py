...
async def polls_command(ctx, *, arguments: str = ""):
    ...
    Usage:
      -polls "Question" "duration" "Answer 1" "Answer 2" ["Answer 3"]

    Answer 3 is optional and can be omitted or left as "".
    """
    import shlex
    ...
    try:
        ...
        await ctx.send(
            '❌ Invalid quotes. Put each field in quotes, for example: '
            '`-polls "Do you like this?" "1d" "Yes" "No" "Maybe"`.',
            delete_after=8
        )
        return
    ...
    if len(parts) < 4:
        await ctx.send(
            '❌ Usage: `-polls <question> <duration> <answer1> <answer2> [answer3]`\\n'
            'Example: `-polls "What should we do?" "1d" "Gaming" "Movie" "Other"`\\n'
            'Answer 3 is optional, so this also works: '
            '`-polls "What should we do?" "1d" "Gaming" "Movie"`.',
            delete_after=10
        )
        return

    # The command order is fixed:
    # question -> duration -> answer 1 -> answer 2 -> optional answer 3.
    question = parts[0]
    duration_str = parts[1]
    answer_parts = parts[2:]

    duration_seconds = parse_poll_duration(duration_str)
    ...
    if len(answer_parts) > 3:
        await ctx.send(
            "❌ You can provide only **2 answers**, plus **Answer 3 (optional)**.",
            delete_after=8
        )
        return
    ...
    if len(answer_parts) < 2:
        await ctx.send(
            "❌ You must provide **Answer 1** and **Answer 2**. **Answer 3 is optional**.",
            delete_after=8
        )
        return

    # An explicitly blank Answer 3 (for example, "") is treated as omitted.
    answer_parts = [answer.strip() for answer in answer_parts]
    if len(answer_parts) == 3 and not answer_parts[2]:
    ...
    if not all(answer_parts):
        await ctx.send("❌ Answer 1 and Answer 2 cannot be blank.", delete_after=7)
        return

    # Discord native polls currently require 1 hour to 7 days.
