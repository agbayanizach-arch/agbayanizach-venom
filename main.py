...
class VouchAdminSelect(discord.ui.Select):
    def __init__(self, owner_id: int, admins):
        self.owner_id = owner_id
        self.admins = {str(member.id): member for member in admins}

        options = [
            discord.SelectOption(
                label=member.display_name[:100],
                description=f"Administrator • {member.name}"[:100],
                value=str(member.id),
                emoji="👑"
            )
            for member in admins[:25]
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        target = self.admins.get(self.values[0])
        if target is None:
            await interaction.response.send_message(
        ...
        # Security check in case permissions changed after the menu was created.
...
class VouchTargetView(discord.ui.View):
    def __init__(self, owner_id: int, guild: discord.Guild):
        ...
        # ONLY members with the Discord Administrator permission are added.
        # This is a normal select menu, so non-administrators never appear as options.
        admins = [
        ...
        if admins:
            self.add_item(VouchAdminSelect(owner_id, admins))
        else:
            self.add_item(discord.ui.Select(
                placeholder="No administrators available",
                options=[discord.SelectOption(label="No administrators available", value="none")],
                disabled=True
            ))
