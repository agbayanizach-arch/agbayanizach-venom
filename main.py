...
def render_variables(template: str, member: discord.Member, channel=None, invite=None) -> str:
    ...
    values = {
        ...
        "{invite_regular}": str(inviter_stats.get("regular", 0)),
        "{invite_leaves}": str(inviter_stats.get("leaves", 0)),
        "{invite_fake}": str(inviter_stats.get("fake", 0)),
        "{invite_bonus}": str(inviter_stats.get("bonus", 0)),
        "{invite_total}": str(int(inviter_stats.get("regular", 0)) + int(inviter_stats.get("bonus", 0))),
    }
...
async def on_member_join(member):
    ...
    if target_channel_id:
        ...
        if greet_channel:
            ...
            try:
                # /welcome-delafter uses the configured delete_after value.
                delete_after = int(guild_welcome.get("delete_after", 0) or 0)
                sent_message = await greet_channel.send(
