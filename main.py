...
@commands.has_permissions(manage_channels=True)  # Requires user to have permission to delete channels
async def delete_ticket(ctx):
    # Optional: Safety check ensuring they can only use this in text channels that start with 'ticket-'
...
# Error handling for the delete command if a user lacks permissions
@delete_ticket.error
async def delete_ticket_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.send("❌ You do not have permission (`Manage Channels`) to use this command.", delete_after=5)
