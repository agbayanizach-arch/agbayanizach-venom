...
AUTOMODE_OPTIONS = {
    "antinuke": ("🛡️ Anti-Nuke", "Protects against mass destructive actions."),
    "antiraid": ("🚨 Anti-Raid", "Helps protect against sudden join raids."),
    "antispam": ("💬 Anti-Spam", "Helps control repeated message spam."),
    "antilink": ("🔗 Anti-Link", "Blocks unauthorized links."),
    "antibot": ("🤖 Anti-Bot", "Controls newly added bots."),
    "antiwebhook": ("🪝 Anti-Webhook", "Controls unauthorized webhook changes."),
}
...
def build_automode_embed(guild):
    ...
    embed = discord.Embed(
        ...
        description=(
            "Configure your server's automatic security protections below.\n\n"
            + "\n\n".join(lines)
            + "\n\n**Only server Administrators can change these settings.**"
        ),
...
class AutomodeView(discord.ui.View):
    def __init__(self, guild_id: int):
        ...
        for key, (label, _) in AUTOMODE_OPTIONS.items():
            button = discord.ui.Button(
                label=label,
                style=discord.ButtonStyle.success if config.get(key) else discord.ButtonStyle.secondary,
                custom_id=f"automode:{key}:{guild_id}"
            )
    ...
    def _make_callback(self, key):
        async def callback(interaction: discord.Interaction):
            ...
            if not isinstance(interaction.user, discord.Member) or not interaction.user.guild_permissions.administrator:
                await interaction.response.send_message("❌ You need **Administrator** permission to change Automode settings.", ephemeral=True)
                return
...
def build_boost_embed(booster: discord.Member, custom_message: str) -> discord.Embed:
    ...
    rendered_message = rendered_message.replace("{user}", booster.mention)
    rendered_message = rendered_message.replace("{mention}", booster.mention)
    rendered_message = rendered_message.replace("{username}", booster.name)
    rendered_message = rendered_message.replace("{displayname}", booster.display_name)
    rendered_message = rendered_message.replace("{server}", booster.guild.name)
...
async def testboostmessage(interaction: discord.Interaction):
    ...
    embed = build_boost_embed(interaction.user, settings["message"])
...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    embed = build_boost_embed(after, custom_message)
