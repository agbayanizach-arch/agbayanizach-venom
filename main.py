...
class TicketControls(discord.ui.View):
    def __init__(self):
        ...
        config = get_ticket_config(0)
        # Persistent button custom_id is kept stable so old deployed panels continue working.

    @discord.ui.button(label="Create Ticket", emoji="🎫", style=discord.ButtonStyle.green, custom_id="open_ticket_btn")
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Use the member ID in the channel name so duplicate usernames cannot collide.
        ...
        try:
            # Ticket numbers are independent for every server. The persisted
            # value stores the latest ticket number used in that guild.
            guild_key = str(guild.id)
            next_ticket_number = int(ticket_created_counts.get(guild_key, 0) or 0) + 1

            # Avoid collisions if a channel with the next number already
            # exists (for example, after a manual channel rename).
...
class TicketCategorySelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.parent_view.status_label.label = f"Category: {category.name}"
...
class TicketStaffRoleSelect(discord.ui.RoleSelect):
    def __init__(self, parent_view):
        self.parent_view = parent_view
        super().__init__(
            placeholder="Select the staff/support role",
            min_values=1,
            max_values=1,
            row=1
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        role = self.values[0]
        self.parent_view.staff_role_id = role.id
        self.parent_view.staff_role_name = role.name
        self.parent_view.status_label.label = f"Staff Role: {role.name}"
        await interaction.response.edit_message(view=self.parent_view)
...
class TicketConfigModal(discord.ui.Modal, title="Ticket Panel Settings"):
    ...
    def __init__(self, parent_view):
        ...
        self.button_label.default = config.get("button_label", "Create Ticket")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        self.parent_view.button_label = str(self.button_label.value)
...
class TicketConfigView(discord.ui.View):
    def __init__(self, guild_id, panel_channel):
        ...
        old = get_ticket_config(guild_id)
        self.category_id = old.get("category_id") or ticket_categories.get(str(guild_id))
        self.staff_role_id = old.get("staff_role_id")
        self.staff_role_name = "Configured role" if self.staff_role_id else "Not selected"
        self.panel_title = old.get("panel_title", "Support Center")
        self.panel_description = old.get("panel_description", "Need help? Click the button below to open a private support ticket.")
        self.button_label = old.get("button_label", "Create Ticket")
        self.ticket_message = old.get("ticket_message", "Please describe your issue or inquiry here. Support staff will assist you shortly.")
        self.status_label = discord.ui.Button(label="Choose your options above", style=discord.ButtonStyle.secondary, disabled=True, row=4)
        self.add_item(TicketCategorySelect(self))
        self.add_item(TicketStaffRoleSelect(self))
        self.add_item(self.status_label)

    @discord.ui.button(label="Edit Text", emoji="✏️", style=discord.ButtonStyle.primary, row=2)
    ...
    @discord.ui.button(label="Deploy Ticket Panel", emoji="🚀", style=discord.ButtonStyle.success, row=3)
    async def deploy(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not self.staff_role_id:
            await interaction.response.send_message("❌ Select a staff/support role first.", ephemeral=True)
            return
        ...
        if not isinstance(category, discord.CategoryChannel) or staff_role is None:
            await interaction.response.send_message("❌ The selected category or role no longer exists.", ephemeral=True)
            return
        ...
        # Ticket numbering is tracked internally per server, but the panel
        # intentionally does not display a Created Tickets counter.
        embed.set_footer(text="Ticket System • Private support tickets")
        view = TicketControls()
        # The persistent view uses a fixed custom_id, so update the visible label for this deployment.
        view.children[0].label = self.button_label[:80]
        await self.panel_channel.send(embed=embed, view=view)
        await interaction.response.edit_message(
            content=f"✅ Ticket panel deployed in {self.panel_channel.mention}.\n\nCategory: {category.mention}\nStaff role: {staff_role.mention}",
            embed=None,
            view=None
        )
        self.stop()
