...
AUTOMODE_OPTIONS = {
    ...
    "antispam": ("💬 Anti-Spam", "5 messages in a short burst = 1 hour timeout."),
...
async def handle_automode_antispam(message):
    if not automode_enabled(message.guild, "antispam") or automode_exempt(message.author):
        return False
    now = datetime.datetime.now(datetime.timezone.utc).timestamp()
    key = (message.guild.id, message.author.id)
    timestamps = [t for t in spam_tracker.get(key, []) if now - t <= 8]
    ...
    spam_tracker[key] = []
    try:
        await message.author.timeout(datetime.timedelta(hours=1), reason="Automode Anti-Spam: 5 messages in a short burst")
    except (discord.Forbidden, discord.HTTPException):
        pass
    return True
...
async def on_message(message):
    ...
    if await handle_automode_antilink(message):
        await bot.process_commands(message)
        return
    # Anti-Spam: 5 messages in an 8-second burst -> 1 hour timeout.
