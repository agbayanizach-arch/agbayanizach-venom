...
def has_admin_or_staff_role(ctx):
    """Allow giveaway management for server Administrators or members with a Staff role."""
    ...
    # Prefer the staff role configured by /staffapply, when available.
    settings = staff_apply_settings.get(str(ctx.guild.id), {})
    configured_role_id = settings.get("role_id") if isinstance(settings, dict) else None
    if configured_role_id:
        try:
            configured_role = ctx.guild.get_role(int(configured_role_id))
            if configured_role and configured_role in ctx.author.roles:
                return True
        except (TypeError, ValueError):
            pass

    # Also support a normal role named "Staff" (case-insensitive).
    return any(
        isinstance(role, discord.Role) and role.name.casefold() == "staff"
...
async def end_giveaway(ctx, giveaway_id: str):
    if not has_admin_or_staff_role(ctx):
        await ctx.send(
            "❌ You need the **Administrator** permission or a **Staff** role in this server to use `-gend`.",
            delete_after=5
        )
        return
...
async def reroll_giveaway(ctx, giveaway_id: str):
    if not has_admin_or_staff_role(ctx):
        await ctx.send(
            "❌ You need the **Administrator** permission or a **Staff** role in this server to use `-greroll`.",
            delete_after=5
        )
        return
