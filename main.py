...
class GiveawayView(discord.ui.View):
    ...
    @discord.ui.button(label="0", emoji="🎉", style=discord.ButtonStyle.blurple, custom_id="join_giveaway_btn")
    async def join_giveaway(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id in self.entries:
            await interaction.response.send_message("❌ You have already entered this giveaway!", ephemeral=True)
        else:
            self.entries.append(interaction.user.id)
            button.label = str(len(self.entries))
...
async def start_giveaway(ctx, duration: str, winners: int, *, prize: str):
    ...
    view = GiveawayView()
    giveaway_msg = await ctx.send("**New Giveaway**", embed=embed, view=view)
