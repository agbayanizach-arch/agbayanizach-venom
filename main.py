...
class StaffApplicationModal(discord.ui.Modal, title="Staff Application"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🛡️ Staff Experience", value=self.experience.value, inline=False)
        embed.add_field(name="❓ Why should we accept you?", value=self.why.value, inline=False)
...
@bot.command(name="staffapply")
@commands.has_permissions(administrator=True)
async def staffapply(ctx, *, setup: str):
    """Configure the staff application system.

    Usage: !staffapply <message> <application channel> <role> <review channel>
    The first channel receives applications. The last channel receives accept/reject notifications.
    """
    if ctx.guild is None:
        await ctx.send("❌ This command can only be used in a server.", delete_after=5)
        return

    channel_matches = re.findall(r"<#(\d+)>", setup)
    role_match = re.search(r"<@&([0-9]+)>", setup)
    if len(channel_matches) < 2 or not role_match:
        await ctx.send(
            "❌ Usage: `!staffapply <message> <#application-channel> <@role> <#review-channel>`\n"
            "Example: `!staffapply We are looking for active and helpful staff. #staff-applications @Staff #staff-results`",
            delete_after=12
        )
        return

    application_channel = ctx.guild.get_channel(int(channel_matches[0]))
    review_channel = ctx.guild.get_channel(int(channel_matches[1]))
    role = ctx.guild.get_role(int(role_match.group(1)))

    if not isinstance(application_channel, discord.TextChannel):
        await ctx.send("❌ The application channel is invalid or does not exist.", delete_after=7)
        return
    if not isinstance(review_channel, discord.TextChannel):
        await ctx.send("❌ The review channel is invalid or does not exist.", delete_after=7)
        return
    if role is None:
        await ctx.send("❌ The staff role is invalid or does not exist.", delete_after=7)
        return

    message = setup
    message = message.replace(f"<#{channel_matches[0]}>", "", 1)
    message = message.replace(role_match.group(0), "", 1)
    message = message.replace(f"<#{channel_matches[1]}>", "", 1)
    message = message.strip()
    if not message:
        await ctx.send("❌ Please provide the staff application requirements message.", delete_after=7)
        return

    me = ctx.guild.me
    if me is None or not me.guild_permissions.send_messages or not me.guild_permissions.embed_links:
        await ctx.send("❌ I need **Send Messages** and **Embed Links** permissions.", delete_after=7)
        return
    if not me.guild_permissions.manage_roles:
        await ctx.send("⚠️ I need **Manage Roles** before I can automatically give the staff role when an application is accepted.", delete_after=10)
        return
    if role >= me.top_role:
        await ctx.send("❌ I cannot automatically give that role because it is higher than or equal to my highest role.", delete_after=8)
        return

    # Independent per-server setup.
    ...
    staff_apply_settings[guild_key] = {
        ...
        "applications": staff_apply_settings.get("applications", {})
    }
    save_json_settings(STAFF_APPLY_FILE, staff_apply_settings)
    ...
    embed.set_footer(text=f"Staff role: {role.name} • Applications Open")

    panel_message = await application_channel.send(embed=embed, view=StaffApplyPanelView(ctx.guild.id, disabled=False))
...
async def close_staff_apply(ctx):
    ...
    try:
        ...
        if message:
            embed = message.embeds[0] if message.embeds else discord.Embed(title="🛡️ Staff Applications")
            embed.set_footer(text="Staff Applications • Closed")
...
async def open_staff_apply(ctx):
    ...
    try:
        ...
        if message:
            embed = message.embeds[0] if message.embeds else discord.Embed(title="🛡️ Staff Applications")
            embed.set_footer(text="Staff Applications • Open")
