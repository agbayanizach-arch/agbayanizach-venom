...
async def on_message(message):
    ...
    if configured_channel is not None and message.channel.id == int(configured_channel):
        ...
        # Softban = ban the member (deleting recent messages) and immediately unban.
        if message.guild.me and message.guild.me.guild_permissions.ban_members:
            if member.id != message.guild.owner_id and member.top_role < message.guild.me.top_role:
                try:
                    ...
                    await message.guild.unban(member, reason="Honeypot softban completed")
                    ...
                    # Update the original honeypot embed so the Kicks counter
                    ...
                    print(f"Honeypot could not softban {member} in {message.guild.name}: permission or role hierarchy issue.")
                except discord.HTTPException as e:
                    print(f"Honeypot softban failed in {message.guild.name}: {e}")
        return
...
async def honeypot(interaction: discord.Interaction):
    ...
    if me is None or not me.guild_permissions.ban_members:
        await interaction.response.send_message(
            "❌ I need **Ban Members** permission to run the honeypot softban.",
            ephemeral=True
        )
        return
    ...
    try:
        ...
        honeypot_count = 0
    embed.add_field(name="Kicks", value=f"`{honeypot_count}`", inline=False)
    embed.set_footer(
    ...
    # Save the panel message ID so the Kicks counter can be updated live.
