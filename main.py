    ...
    async def setup_hook(self):
        ...
        for guild_id, settings in staff_apply_settings.items():
            if guild_id == "applications" or not isinstance(settings, dict):
                continue
            try:
                self.add_view(StaffApplyPanelView(int(guild_id)))
            except (TypeError, ValueError):
                continue
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!staffapply <message> <#channel> <@role>` — Creates/configures the staff application panel.\n"
...
async def staffapply(ctx, *, setup: str):
    """Configure the staff application panel.

    Usage: !staffapply <requirements message> <targetchannel> <role>
    The target channel should be a channel mention and the role should be a role mention.
    """
    if ctx.guild is None:
        await ctx.send("❌ This command can only be used in a server.", delete_after=5)
        return

    channel_match = re.search(r"<#(\d+)>", setup)
    role_match = re.search(r"<@&([0-9]+)>", setup)
    if not channel_match or not role_match:
        await ctx.send(
            "❌ Usage: `!staffapply <requirements message> <#targetchannel> <@role>`\n"
            "Example: `!staffapply We are looking for active and helpful staff. #staff-applications @Staff`",
            delete_after=12
        )
        return
    ...
    if not isinstance(target_channel, discord.TextChannel):
        await ctx.send("❌ The target channel is invalid or does not exist.", delete_after=7)
        return
    if role is None:
        await ctx.send("❌ The staff role is invalid or does not exist.", delete_after=7)
        return

    # Remove the channel and role mentions from the requirements message.
    message = setup
    message = message.replace(channel_match.group(0), "")
    message = message.replace(role_match.group(0), "")
    ...
    if not me.guild_permissions.manage_roles:
        await ctx.send("⚠️ The panel can be created, but I need **Manage Roles** before I can automatically give the staff role when an application is accepted.", delete_after=10)
        return
    ...
    staff_apply_settings[str(ctx.guild.id)] = {
        "message": message,
    ...
    embed.set_footer(text=f"Staff role: {role.name}")

    await target_channel.send(embed=embed, view=StaffApplyPanelView(ctx.guild.id))
    try:
        await ctx.message.delete()
    except (discord.Forbidden, discord.HTTPException):
        pass
