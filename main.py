...
class TicketConfigView(discord.ui.View):
    def __init__(self, guild_id, panel_channel):
        ...
        old_role_ids = old.get("staff_role_ids")
        if isinstance(old_role_ids, list):
            self.staff_role_ids = old_role_ids[:25]
        elif old.get("staff_role_id"):
            self.staff_role_ids = [old.get("staff_role_id")]
        else:
            self.staff_role_ids = []
        self.staff_role_id = self.staff_role_ids[0] if self.staff_role_ids else None
        self.staff_role_names = ["Configured role"] if self.staff_role_ids else []
        ...
        if isinstance(old_buttons, list) and old_buttons:
            self.ticket_buttons = [
                ...
                for item in old_buttons[:25]
                if isinstance(item, dict) and str(item.get("label", "")).strip()
            ]
        else:
            self.ticket_buttons = [{
                "label": str(old.get("button_label", "Create Ticket"))[:80],
                "emoji": "🎫",
                "style": discord.ButtonStyle.green.value,
            }]
    ...
    @discord.ui.button(label="➕ Add Button", style=discord.ButtonStyle.primary, row=2)
