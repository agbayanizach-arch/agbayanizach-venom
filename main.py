...
async def ticket_setup(interaction: discord.Interaction, category: discord.CategoryChannel):
    """Configure the ticket category and post the ticket panel in the current channel."""
    try:
        # Acknowledge the slash command immediately so Discord does not keep it
        # stuck on "thinking..." while the panel is being created.
        ...
        if channel is None or not hasattr(channel, "send"):
            await interaction.followup.send(
                "❌ I cannot post a ticket panel in this channel. Please run the command in a normal text channel.",
                ephemeral=True
            )
            return
        ...
        if me is None:
            await interaction.followup.send(
                "❌ I could not verify my bot permissions. Please try again.",
                ephemeral=True
            )
            return

        # Check permissions in the ACTUAL channel where /ticket-setup was used.
        # Manage Channels is not required just to post the panel; it is only
        # needed later when a member clicks Create Ticket and a channel is made.
        ...
        if missing:
            await interaction.followup.send(
                "❌ I cannot post the ticket panel here. Missing: "
        ...
        # Make sure the selected category belongs to this same server.
        ...
        # Send the panel FIRST. Only save the category if Discord accepted it.
        await channel.send(
            embed=embed,
            view=TicketControls()
        )
        ...
        # This also catches permission failures from channel.send/create-related checks.
        if interaction.response.is_done():
            await interaction.followup.send(
                "❌ Discord denied the ticket panel. Please give the bot **View Channel**, "
                "**Send Messages**, and **Embed Links** permission in this channel.",
                ephemeral=True
            )
        return
        ...
        if interaction.response.is_done():
            await interaction.followup.send(
                "❌ Discord rejected the ticket panel. Please try `/ticket-setup <category>` again.",
                ephemeral=True
            )
        return
        ...
        if interaction.response.is_done():
            await interaction.followup.send(
                "❌ An unexpected error occurred while setting up the ticket system. "
...
async def ticket_setup_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    """Always finish the interaction when /ticket-setup itself raises an app-command error."""
    if isinstance(error, app_commands.MissingPermissions):
        message = "❌ You need **Administrator** permission to use `/ticket-setup`."
    else:
        print(f"/ticket-setup command error: {error}")
        message = "❌ `/ticket-setup` failed. Check the bot console for the exact error."
