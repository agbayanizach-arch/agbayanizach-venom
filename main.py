...
# Animated giveaway reaction emoji (Falcon-style). Discord displays its live participant count automatically.
GIVEAWAY_EMOJI = discord.PartialEmoji(name="tada", id=1046529384756291645, animated=True)
...
async def finish_giveaway(giveaway_id: str):
    ...
    for reaction in giveaway_msg.reactions:
        if str(reaction.emoji) != str(GIVEAWAY_EMOJI):
            continue
        try:
...
async def start_giveaway(ctx, duration: str, winners: int, *, prize: str):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"🔹 **React with {GIVEAWAY_EMOJI} to participate!**"
        ),
        color=giveaway_color
    )
