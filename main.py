...
class VouchAdminButton(discord.ui.Button):
    def __init__(self, owner_id: int, target: discord.Member):
        super().__init__(
            label=target.display_name[:80],
            style=discord.ButtonStyle.secondary,
            custom_id=f"vouch_admin_profile:{owner_id}:{target.id}"
        )
...
class VouchTargetView(discord.ui.View):
    def __init__(self, owner_id: int, guild: discord.Guild):
        ...
        # Each administrator is shown with their real Discord profile avatar.
        admins = [
        ...
        # One profile embed + one button per administrator.
        # This keeps the selector administrator-only while displaying each
        # administrator's actual Discord profile/avatar.
        for admin in admins[:25]:
            embed = discord.Embed(
                title=admin.display_name,
                description=f"Administrator • {admin.name}",
                color=discord.Color.blurple()
            )
            embed.set_thumbnail(url=admin.display_avatar.url)
            self.add_item(VouchAdminButton(owner_id, admin))
            self._profile_embeds = getattr(self, "_profile_embeds", [])
            self._profile_embeds.append(embed)

    @property
    def profile_embeds(self):
        return getattr(self, "_profile_embeds", [])
...
class VouchResultView(discord.ui.View):
    ...
    async def submit_vouch(self, interaction: discord.Interaction, button: discord.ui.Button):
        target_view = VouchTargetView(interaction.user.id, interaction.guild)
        if not target_view.profile_embeds:
            await interaction.response.send_message(
        ...
        await interaction.response.send_message(
            ...
            embeds=target_view.profile_embeds,
            view=target_view,
            ephemeral=True
        )
