...
class VouchTargetView(discord.ui.View):
    def __init__(self, owner_id: int, guild: discord.Guild):
        ...
        # Only members with the Administrator permission are shown as vouch targets.
        admin_members = [
            member for member in guild.members
            if not member.bot and member.guild_permissions.administrator
        ]
        admin_members.sort(key=lambda member: member.display_name.lower())

        # Discord select menus allow a maximum of 25 options.
        admin_members = admin_members[:25]
        ...
        if options:
            select = discord.ui.Select(
                placeholder="Select the administrator you want to vouch for",
                min_values=1,
                max_values=1,
                options=options,
                custom_id=f"vouch_admin_select:{owner_id}"
            )
            select.callback = self.target_select
            self.add_item(select)
        else:
            self.add_item(discord.ui.Button(
                label="No administrators available",
                style=discord.ButtonStyle.secondary,
                disabled=True
            ))

    async def target_select(self, interaction: discord.Interaction):
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message("❌ This vouch form belongs to another member.", ephemeral=True)
            return

        selected_id = int(interaction.data["values"][0])
        target = interaction.guild.get_member(selected_id) if interaction.guild else None
        if target is None or target.bot or not target.guild_permissions.administrator:
            await interaction.response.send_message(
                "❌ You can only vouch for a member with the **Administrator** permission.",
                ephemeral=True
            )
            return
...
class VouchResultView(discord.ui.View):
    ...
    async def submit_vouch(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            ...
            view=VouchTargetView(interaction.user.id, interaction.guild),
            ephemeral=True
        )
