...
async def handle_automode_antispam(message):
    ...
    key = (message.guild.id, message.author.id)
    timestamps = [t for t in spam_tracker.get(key, []) if now - t <= 1]
    ...
    spam_tracker[key] = []
    try:
        await message.author.timeout(datetime.timedelta(hours=1), reason="Automode Anti-Spam: 5 messages within 1 second")
    except (discord.Forbidden, discord.HTTPException):
        pass
    return True
...
async def on_message(message):
    ...
    if await handle_automode_antilink(message):
        await bot.process_commands(message)
        return
    # Anti-Spam: 5 messages within 1 second -> 1 hour timeout.
