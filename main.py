...
class StaffAdditionalRequirementsModal(discord.ui.Modal):
    def __init__(self, guild_id: int, target_channel_id: int, role_id: int, answers: dict[str, str], requirements: list[str], start_index: int = 0):
        end_index = min(start_index + 5, len(requirements))
        first_question = start_index + 6
        last_question = end_index + 5
        if first_question == last_question:
            modal_title = f"Staff Application • Question {first_question}"
        else:
            modal_title = f"Staff Application • Questions {first_question}-{last_question}"
        ...
        for index, requirement in enumerate(requirements[start_index:start_index + 5], start=start_index + 1):
            ...
            field = discord.ui.TextInput(
                label=f"Q{question_number}: {short_label}"[:45],
