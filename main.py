...
class MyBot(commands.Bot):
    def __init__(self):
        super().__init__(command_prefix="-", case_insensitive=True, strip_after_prefix=True, intents=discord.Intents.all())
...
async def start_giveaway(ctx, duration: str, winners: int, *, prize: str):
    ...
    # Falcon™ style: "New Giveaway" is outside the embed.
    embed = discord.Embed(
        title=f"🎁 {prize} 🎁",
        description=(
            f"🔹 **Winners:** {winners}\n"
    ...
    end_time_display = datetime.datetime.fromtimestamp(end_timestamp, datetime.timezone(datetime.timedelta(hours=8))).strftime("%B %d, %Y %I:%M %p")
    ...
    # Store the giveaway by its Discord message ID so it can be rerolled later.
    ...
    # Add a real Discord reaction. Members click it to participate.
    try:
        await giveaway_msg.add_reaction("🎉")
    except discord.Forbidden:
        await ctx.send("❌ I need **Add Reactions** permission to run giveaways.", delete_after=10)
        return
    except discord.HTTPException:
        await ctx.send("❌ Discord could not add the giveaway reaction.", delete_after=10)
        return

    await asyncio.sleep(seconds)

    # Re-fetch the message so the reaction list is current.
    try:
    ...
    if not entry_ids:
        ...
        ended_time_display = datetime.datetime.fromtimestamp(ended_timestamp, datetime.timezone(datetime.timedelta(hours=8))).strftime("%B %d, %Y %I:%M %p")
        ...
        giveaway_records[str(giveaway_msg.id)]["ended"] = True
        giveaway_records[str(giveaway_msg.id)]["winner_ids"] = []
        save_json_settings(GIVEAWAY_FILE, giveaway_records)
        return
    ...
    # Keep the real reaction visible; only remove the interactive button system (there is none).
    await giveaway_msg.edit(
        content="**Giveaway Ended**",
        embed=end_embed,
        allowed_mentions=discord.AllowedMentions(users=True)
    ...
    giveaway_records[str(giveaway_msg.id)]["ended"] = True
    giveaway_records[str(giveaway_msg.id)]["winner_ids"] = winning_ids
    save_json_settings(GIVEAWAY_FILE, giveaway_records)
