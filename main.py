...
class SuggestionView(discord.ui.View):
    def __init__(self, suggestion_id: int):
        ...
        # Every suggestion gets unique component IDs so multiple suggestion
        # panels can coexist and keep working after a restart.
        if len(self.children) >= 2:
            self.children[0].custom_id = f"suggestion_yes:{self.suggestion_id}"
            self.children[1].custom_id = f"suggestion_no:{self.suggestion_id}"
    ...
    async def _vote(self, interaction: discord.Interaction, vote: str):
        ...
        await interaction.response.send_message(
            f"✅ Your vote has been recorded as **{'Yes 👍' if vote == 'yes' else 'No 👎'}**.",
            ephemeral=True
        )
...
def build_suggestion_embed(message: discord.Message, image_url: str | None = None) -> discord.Embed:
    embed = discord.Embed(
        description=message.content or "*(No text — attachment only)*",
...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        # Restore the persistent Yes/No buttons for saved suggestions after restart.
        for key in suggestions_settings:
            if key.startswith("suggestion:"):
                try:
                    self.add_view(SuggestionView(int(key.split(":", 1)[1])))
        ...
        # Restore persistent ticket categories after a bot restart.
        for guild_id, category_id in ticket_categories.items():
            try:
                int(guild_id)
                int(category_id)
            except (TypeError, ValueError):
...
@app_commands.describe(
    ...
    image="Optional image shown on every suggestion embed."
)
...
async def on_message(message):
    ...
    if not message.author.bot and isinstance(suggestion_config, dict):
        ...
        if configured_channel_id and message.channel.id == int(configured_channel_id):
            ...
            # If the message contains an image attachment, use it automatically.
            if not image_url:
            ...
            if last_submission is not None and now_ts - last_submission < cooldown_seconds:
                ...
                try:
                    ...
                    print(f"Could not send suggestion cooldown warning in {message.guild.name}: {exc}")
                return
