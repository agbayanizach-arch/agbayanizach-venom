...
# Giveaways use Discord's native 🎉 reaction instead of a button.
# This lets Discord highlight the reaction for each member who participates.
...
async def polls_command(ctx, *, arguments: str = ""):
    """Create a native Discord poll with 2 answers, or an optional 3rd answer."""
    import shlex
    ...
    # The duration can be placed first or last.
    # Answer 3 is optional, so both of these are valid:
    # !polls Yes No 24h
    # !polls Yes No Maybe 24h
    # !polls 24h Yes No
    # !polls 24h Yes No Maybe
    if len(parts) < 3:
        await ctx.send(
            "❌ Usage: `!polls <answer1> <answer2> [answer3] <duration>`\n"
            "Example: `!polls Yes No 24h`\n"
            "With a third answer: `!polls Yes No Maybe 24h`.",
            delete_after=10
        )
        return
    ...
    if not duration_positions:
        await ctx.send(
            "❌ Please provide a valid duration such as `1h`, `4h`, `24h`, or `7d`.",
            delete_after=8
        )
        return
    ...
    # Remove the duration. What remains must be exactly 2 or 3 answers.
    answer_parts = parts[:duration_index] + parts[duration_index + 1:]

    # An explicitly empty third answer is also treated as omitted.
    if len(answer_parts) == 3 and not answer_parts[2].strip():
        answer_parts = answer_parts[:2]

    if len(answer_parts) not in (2, 3):
        await ctx.send(
            "❌ Please provide **2 answers**. **Answer 3 is optional**.",
            delete_after=8
        )
        return

    # Discord's native Poll is the exact built-in poll UI.
...
# 🎉 GIVEAWAY COMMAND — FALCON™ FORMAT
...
async def start_giveaway(ctx, duration: str, winners: int, *, prize: str):
    ...
    # Falcon™ giveaway color — the same color is used before and after the giveaway.
    ...
    # Falcon™ active giveaway formatting.
    embed = discord.Embed(
        description=(
            f"🎁 **{prize}** 🎁\n\n"
            f"🔹 **Winners:** {winners}\n"
    ...
    embed.set_footer(text=f"Ends at • <t:{end_timestamp}:F>")

    # Send the Falcon™ giveaway message, then add a real Discord reaction.
    # Members can click 🎉 and Discord will visually highlight their reaction.
    giveaway_msg = await ctx.send("**New Giveaway**", embed=embed)
    await giveaway_msg.add_reaction("🎉")

    await asyncio.sleep(seconds)

    # Refresh the message so we read the latest reaction state.
    giveaway_msg = await ctx.channel.fetch_message(giveaway_msg.id)

    # Collect everyone who is still reacting with 🎉. Bots are ignored.
    entry_ids = []
    ...
    # Remove duplicate user IDs just in case.
    ...
    if not entry_ids:
        ...
        end_embed.set_footer(text=f"Ended at • <t:{ended_timestamp}:F>")
        await giveaway_msg.edit(content="**Giveaway Ended**", embed=end_embed)
        return
    ...
    # Falcon™ ended formatting: congratulate the winners only.
    # No separate DM/claim message is sent.
    end_embed = discord.Embed(
        description=(
            f"🎁 **{prize}** 🎁\n\n"
    ...
    # Edit the original giveaway message. No separate claim message.
    await giveaway_msg.edit(content="**Giveaway Ended**", embed=end_embed, view=None)
