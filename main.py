...
async def ticket_setup(interaction: discord.Interaction, category: discord.CategoryChannel):
    # IMPORTANT: acknowledge the Discord interaction immediately.
    # Discord gives slash commands only a few seconds to respond.
    ...
    # The bot needs these permissions in the channel where the panel is posted.
    required = [
    ...
    try:
        ...
        if channel is None:
            await interaction.followup.send(
                "❌ I could not find the channel where the ticket panel should be posted.",
                ephemeral=True
            )
            return
        ...
        # Because the interaction was deferred, always finish with followup.
        await interaction.followup.send(
        ...
        await interaction.followup.send(
            "❌ I don't have permission to post the ticket panel here. "
            "Please give the bot **Send Messages** and **Embed Links** permission "
            "in this channel.",
            ephemeral=True
        )

    except discord.HTTPException as e:
        print(f"Ticket setup Discord error in guild {interaction.guild.id}: {e}")
        await interaction.followup.send(
            "❌ Discord rejected the ticket panel. Please check the bot permissions "
            "and try `/ticket-setup <category>` again.",
            ephemeral=True
        )

    except Exception as e:
        print(f"Ticket setup unexpected error in guild {interaction.guild.id}: {e}")
        await interaction.followup.send(
